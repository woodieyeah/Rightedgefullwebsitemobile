...
Resend has real, direct email engagement data — open rate, click rate, and exactly which links got clicked per broadcast (confirmed: ~49% open rate, ~2.4% click rate across recent sends). **Important:** Resend's API returns timestamps in raw UTC — both diagnosis jobs are built to explicitly convert to AEST before reasoning about "what day"/"what time" something was sent (a real bug where a Wednesday 1pm AEST send was misreported as "Tuesday 3am" has been fixed).
