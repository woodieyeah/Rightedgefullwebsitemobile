...
app.post("/create-checkout-session", async (c) => {
  try {
    ...
    const email = body?.email?.trim()?.toLowerCase();
    const returnUrl = body?.returnUrl || "http://localhost:5173";
    const returnHash = ["best-bets", "try-scorers", "sgm-builder"].includes(body?.returnHash)
      ? body.returnHash
      : "best-bets";
...
app.post("/confirm-checkout-session", async (c) => {
  try {
    ...
    if (String(sessionId).startsWith("cs_rightedge_")) {
      ...
      const returnHash = ["best-bets", "try-scorers", "sgm-builder"].includes(data.returnHash)
        ? data.returnHash
        : "best-bets";
    ...
    const returnHash = ["best-bets", "try-scorers", "sgm-builder"].includes(session.metadata?.returnHash || "")
