...
var ConversationsPersistence = /** @class */ (function () {
    ...
    ConversationsPersistence.prototype._migrateFromLegacyPersistence = function () {
        ...
        try {
            ...
            var widgetSessionId = persistence.get_property(constants_1.CONVERSATIONS_LEGACY_WIDGET_SESSION_ID);
            if (!widgetSessionId) {
            ...
            var ticketId = persistence.get_property(constants_1.CONVERSATIONS_LEGACY_TICKET_ID);
            if (ticketId) {
                data.ticketId = ticketId;
            }
            var widgetState = persistence.get_property(constants_1.CONVERSATIONS_LEGACY_WIDGET_STATE);
            if (widgetState === 'open' || widgetState === 'closed') {
                data.widgetState = widgetState;
            }
            var userTraits = persistence.get_property(constants_1.CONVERSATIONS_LEGACY_USER_TRAITS);
            if (userTraits) {
                data.userTraits = userTraits;
            }
            this._write(data);
            persistence.unregister(constants_1.CONVERSATIONS_LEGACY_WIDGET_SESSION_ID);
            persistence.unregister(constants_1.CONVERSATIONS_LEGACY_TICKET_ID);
            persistence.unregister(constants_1.CONVERSATIONS_LEGACY_WIDGET_STATE);
            persistence.unregister(constants_1.CONVERSATIONS_LEGACY_USER_TRAITS);
    ...
    ConversationsPersistence.prototype._readLegacyFromRawStorage = function () {
        ...
        try {
            ...
            var widgetSessionId = parsed === null || parsed === void 0 ? void 0 : parsed[constants_1.CONVERSATIONS_LEGACY_WIDGET_SESSION_ID];
            if (typeof widgetSessionId !== 'string' || !widgetSessionId) {
                return null;
            }
            var data = { widgetSessionId: widgetSessionId };
            var ticketId = parsed === null || parsed === void 0 ? void 0 : parsed[constants_1.CONVERSATIONS_LEGACY_TICKET_ID];
            if (ticketId) {
                data.ticketId = ticketId;
            }
            var widgetState = parsed === null || parsed === void 0 ? void 0 : parsed[constants_1.CONVERSATIONS_LEGACY_WIDGET_STATE];
            if (widgetState === 'open' || widgetState === 'closed') {
                data.widgetState = widgetState;
            }
            var userTraits = parsed === null || parsed === void 0 ? void 0 : parsed[constants_1.CONVERSATIONS_LEGACY_USER_TRAITS];
            if (userTraits) {
                data.userTraits = userTraits;
            }
            return data;
